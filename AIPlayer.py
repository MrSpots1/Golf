...
class AIPlayer(Player):
    def __init__(self, maxToleratedScore: int = 10, maxToleratedCard: int = 5):
        ...
        self._maxToleratedScore = maxToleratedScore  # AI will try to keep score below this value
        self._maxToleratedCard = maxToleratedCard  # AI will try to avoid cards with value above this
        self._cardTracker = CardTracker(2)  # assuming 2 decks in the game
    ...
    def observeGameState(self, gameState: GameState) -> None:
        # Observe all cards in the initial game state
        for player in gameState.players:
            for row in range(2):
                for col in range(3):
                    slot = player.hand.getSlot(row=row, column=col)
                    if not slot.isFaceDown:
                        self._cardTracker.observeCard(slot.card)

        # observe the top discard
        self._cardTracker.observeCard(gameState.cardDeck.topDiscardCard())
    ...
    def considerDiscardCard(self, gameState: GameState, card: Card) -> CardSlot | None:
        ...
        return self._analyzeCardPotential(card, facedownCount, currentScore)
    ...
    def watchDiscard(self, gameState: GameState, slot: CardSlot | None, slotIsFacedown: bool, discardCard: Card | None, newDiscard: Card | None) -> None:
        ...
        # if a facedown card was discarded, observe it
        if not slot is None and slotIsFacedown:
            self._cardTracker.observeCard(newDiscard) 
        return 
    ...
    def watchDraw(self, gameState: GameState, slot: CardSlot | None, slotIsFacedown: bool, drawnCard: Card, newDiscard: Card) -> None:
        ...
        # observe the drawn card
        self._cardTracker.observeCard(drawnCard)
    ...
    def _analyzeCardPotential(self, card: Card, unrevealedCount: int, currentScore: int) -> CardSlot | None:
        worstCardValue = -100
        worstSlot = None
        ...
        if worstSlot is not None and fakeSlot.get_card_value() <= self._maxToleratedCard and fakeSlot.get_card_value() < worstCardValue:
            return worstSlot
    ...
    def _considerDiscarding(self, card: Card) -> bool:
        return True
